...
VOID SetFunctionArgument(IN PCONTEXT pThreadCtx, IN ULONG_PTR uValue, IN DWORD dwParmIndex) {
	...
	switch (dwParmIndex) {
	case 0x01:
		pThreadCtx->Rcx = (ULONG_PTR)uValue;
		return;
	case 0x02:
		pThreadCtx->Rdx = (ULONG_PTR)uValue;
		return;
	case 0x03:
		pThreadCtx->R8 = (ULONG_PTR)uValue;
		return;
	case 0x04:
		pThreadCtx->R9 = (ULONG_PTR)uValue;
		return;
	default:
		break;
	}
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
//			InitHardwareBreakpointHooking & CleanUpHardwareBreakpointHooking : PUBLIC
...
BOOL CleanUpHardwareBreakpointHooking() {
	...
	while (TRUE) {
		...
		if (SystemProcInfo->UniqueProcessId == _GetCurrentProcessId()) {
			...
			for (DWORD i = 0; i < SystemProcInfo->NumberOfThreads; i++) {
				...
				if (!SetHardwareBreakpoint((DWORD)SystemThreadInfo[i].ClientId.UniqueThread, Address, Drx, bInitializeHWBP))
					goto _EndOfFunc;
			}
...
BOOL InstallHooksOnNewThreads(IN DRX Drx) {
	...
	g_pNtCreateThreadEx = (PVOID)GetProcAddress(hntdll, "NtCreateThreadEx");
	if (!g_pNtCreateThreadEx)
		return FALSE;

	return InstallHardwareBreakingPntHook((PUINT_VAR_T)g_pNtCreateThreadEx, Drx, NtCreateThreadExDetour, ALL_THREADS);
}


BOOL RemoveHooksOnNewThreads() {
	// Remove the NtCreateThreadEx hook from all the threads
	return RemoveHardwareBreakingPntHook((PUINT_VAR_T)g_pNtCreateThreadEx, ALL_THREADS);
}
